...
def get_train_transform():
    return A.Compose([
        A.Resize(512, 512),
        A.Flip(p=0.5),
        # A.Emboss(always_apply=False, p=1.0, alpha=(0.2, 0.5), strength=(0.0, 1.71)),
        # A.GaussNoise(always_apply=False, p=1.0, var_limit=(10.0, 52.01), per_channel=True, mean=0.0),
        ToTensorV2(p=1.0)
...
def train_fn(num_epochs, train_data_loader, optimizer, model, device, clip=35):
    ...
    for epoch in range(num_epochs):
        ...
        torch.save(model.state_dict(), f'epoch_{epoch+1}_tf_d2_ap.pth')

def main():
    annotation = '../dataset/skfold/cv_train_skfold1.json'
    data_dir = '../dataset'
